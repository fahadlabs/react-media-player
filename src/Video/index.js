...
import { Slider, withStyles, IconButton, Typography, Tooltip } from '@material-ui/core';
  ...
  return (
    ...
    >
      <video
        ...
        src={src}
        loop={repeat}
        onEndedCapture={() => setPaused(true)}
        onLoad={() => console.log(video.duration)}
        onLoadedDataCapture={() => setDuration(video.duration)}
        onTimeUpdateCapture={() => setCtime((100 / video.duration) * video.currentTime)}
      />
      <div style={{ position: 'absolute', width: '100%', height: '100%', top: 0, left: 0, display: 'flex', flexDirection: 'column' }}>
        {!!title && (
          <div style={{ backgroundColor: '#0008', opacity: active || paused ? 1 : 0 }}>
            <Tooltip
              title={
                <Fragment>
        ...
        >
          {paused && <PlayCircleOutline style={{ color: '#fff', fontSize: 50, backgroundColor: '#0005', borderRadius: '50%' }} />}
        </div>
        <div style={{ backgroundColor: '#0008', opacity: active || paused ? 1 : 0 }}>
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ flex: 1, textAlign: 'center', fontSize: 18 }}>{format(~~((duration / 100) * ctime))}</div>
            <div style={{ flex: 5, display: 'flex' }}>
              <Duration
                valueLabelDisplay='auto'
                valueLabelFormat={format(~~((duration / 100) * ctime))}
                value={~~ctime}
                onChange={onChangeDuration}
              />
            </div>
            <div style={{ flex: 1, textAlign: 'center', fontSize: 18 }}>{format(~~duration)}</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', flex: 1, alignItems: 'center' }}>
              ...
              <div style={{ flex: 1, textAlign: 'center' }}>
                ...
                <IconButton style={{ padding: '2px 5px' }} onClick={toggleFullscreen}>
                  {fullscreen ? <CropSquare style={{ color: white, fontSize: 22 }} /> : <CropFree style={{ color: white, fontSize: 22 }} />}
                </IconButton>
              </div>
            </div>
          </div>
